...
legacy_generate() {
    ...
    local tmp_full
    tmp_full=$(mktemp)
    printf "<!-- Generated by docsh/70_generate_sidebar.sh -->\n" >> "$tmp_full"
    printf "<!-- include_root=%s -->\n\n" "$include_root"        >> "$tmp_full"
    ...
    cat "$tmp_out" >> "$tmp_full"
    rm -f "$tmp_out"

    # Strip the datestamp line from the existing file before comparing so
    # a timestamp-only difference does not count as a real change.
    local tmp_cur
    tmp_cur=$(mktemp)
    grep -v '^<!-- Generated by docsh.*-->$' "$sidebar_file" > "$tmp_cur" 2>/dev/null || true

    if cmp -s "$tmp_full" "$tmp_cur"; then
        echo "  Unchanged: ${sidebar_file#${REPO_ROOT}/}"
        rm -f "$tmp_full" "$tmp_cur"
        return 0
    fi
    rm -f "$tmp_cur"
    mv "$tmp_full" "$sidebar_file"

    echo "  Updated: ${sidebar_file#${REPO_ROOT}/}"
}
